...
class G1ThreePolicyNode(UnitreeRsCameraNode):
    ...
    def start_ros_handlers(self):
        ...
        self.get_logger().info("Starting three-policy loop at 50 Hz.")
    ...
    def main_loop_callback(self):
        ...
        elif self.joy_stick_data.A and self.current_agent_name != "walk":
            self._switch_to("walk", "A pressed, switching to 29DoF walk.")
        elif self.joy_stick_data.L1 and self.current_agent_name != "parkour":
            self._switch_to("parkour", "L1 pressed, switching to 31DoF parkour.")

        self._log_main_loop_frequency(start_time)

    def _send_agent_action(self, agent_name, action):
...
def main(args):
    ...
    walk_agent = Body29ActorOn31Agent(
        ...
        ros_node=node,
        x_vel_scale=args.walk_x_vel_scale,
        y_vel_scale=args.walk_y_vel_scale,
        yaw_vel_scale=args.walk_yaw_vel_scale,
    )
    parkour_agent = ParkourAgent(
        logdir=args.parkour_logdir,
        ros_node=node,
        depth_vis=args.depth_vis,
        pointcloud_vis=args.pointcloud_vis,
        lin_vel_deadband=args.parkour_lin_vel_deadband,
    ...
    node.get_logger().info("G1ThreePolicyNode is ready. R1=stand, A=walk, L1=parkour.")
    try:
        rclpy.spin(node)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="G1 deployment node: 29DoF stand, 29DoF walk, 31DoF parkour")
    parser.add_argument("--stand_logdir", type=str, required=True, help="29DoF stand policy directory")
    parser.add_argument("--walk_logdir", type=str, required=True, help="29DoF no-vision walk policy directory")
    parser.add_argument("--parkour_logdir", type=str, required=True, help="31DoF visual parkour policy directory")
    ...
    parser.add_argument("--walk_x_vel_scale", type=float, default=0.5)
    parser.add_argument("--walk_y_vel_scale", type=float, default=0.5)
    parser.add_argument("--walk_yaw_vel_scale", type=float, default=1.0)
    parser.add_argument("--parkour_lin_vel_deadband", type=float, default=0.5)
